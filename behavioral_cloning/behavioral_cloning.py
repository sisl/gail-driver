...
def main():
    ...
    parser.add_argument('--data_file',          type= str,  default='0750am-0805am', help='time slot for data file')
    parser.add_argument('--data_dir',           type= str,  default='./2d_drive_data/', help='directory containing data')

    parser.add_argument('--batch_size',         type=int,   default= 64,        help='minibatch size')
    parser.add_argument('--seq_length',         type=int,   default= 100,       help='training sequence length')
    parser.add_argument('--state_dim',          type=int,   default=  51,       help='number of state variables')
    parser.add_argument('--extract_temporal',   type=bool,  default=  False,    help='Whether to extract temporal features')
    ...
    parser.add_argument('--h5_name',            type=str,   default='',         help='Name for saved h5 file')

    ############################
    #       Policy Network     #
    ############################
    parser.add_argument('--mlp_activation',  default=tf.nn.elu, help='activation function in policy network')
    parser.add_argument('--mlp_size',           type=int, nargs='+',   default=[],        help='number of neurons in each feedforward layer')
    parser.add_argument('--gru_input_dim',      type=int,   default= 64,        help='size of input to gru layer')
    parser.add_argument('--gru_size',           type=int,   default= 64,        help='size of gru layer')
    parser.add_argument('--policy_varscope',    type=str,   default='policy',   help='variable scope for policy network')
...
# Train network
def train(args, net):
    data_loader = DataLoader(args.batch_size, args.val_frac, args.seq_length, args.extract_temporal)
        ...
        for e in xrange(args.num_epochs):
            ...
            for b in xrange(data_loader.n_batches_train):
                ...
                for t in xrange(args.seq_length):
                    ...
                    if args.gru_input_dim > 0:
                        feed_in[net.hprev] = hprev
                        feed_out = [net.cost, net.h, net.summary_policy, net.train]
                        train_loss, hprev, summary_policy, _ = sess.run(feed_out, feed_in)
                    else:
                        feed_out = [net.cost, net.summary_policy, net.train]
                        train_loss, summary_policy, _ = sess.run(feed_out, feed_in)
