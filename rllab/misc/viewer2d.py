...
class Viewer2D(object):
    ...
    def circle(self, color, p, radius):
        pygame.draw.circle(self.screen, color, self.scale_point(
            p), int(self.scale_size(radius)))
    ...
    def polygon(self, color, points):
        if len(color) > 3:
            ...
            pygame.draw.polygon(self.screen, color, list(
                map(self.scale_point, points)))
    ...
    def checker(self, colors=[Colors.white, Colors.black], granularity=4, offset=(0, 0)):
        ...
        for row in range(-1, int(np.ceil(screen_height * 1.0 / checker_size)) + 1):
            for col in range(-1, int(np.ceil(screen_width * 1.0 / checker_size)) + 1):
                the_square = (col * checker_size + offset_x, row *
                              checker_size + offset_y, checker_size, checker_size)
                self.screen.fill(
                    colors[(start_idx + row + col) % 2], the_square)
